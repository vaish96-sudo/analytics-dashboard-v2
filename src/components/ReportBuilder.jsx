  ...
  return (
    <div onDragOver={(e) => { e.preventDefault(); setDragOver(true) }} onDragLeave={() => setDragOver(false)}
      ...
      <div className="flex items-center gap-1.5 flex-wrap">
        ...
        {items.map(item => (
          <span key={item.col} className={`inline-flex items-center gap-1 px-2 py-1 rounded-md border text-xs font-medium ${item.type === 'dimension' ? 'bg-sky-50 text-sky-700 border-sky-200' : 'bg-emerald-50 text-emerald-700 border-emerald-200'}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div><p className="text-xs font-medium text-slate-500 mb-1.5">Selected Metrics</p>
              <DropZone label="Metrics" items={selectedMetrics.map(c => ({ col: c, label: schema[c]?.label || c, type: 'metric' }))} onDrop={addMetric} onRemove={removeMetric} accept={['metric']} /></div>
          </div>
