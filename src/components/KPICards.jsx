...
export default function KPICards() {
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
      {kpis.map((kpi, i) => (
        <div key={kpi.col} className={`p-4 rounded-xl bg-white border-2 ${kpi.isCustom ? 'border-teal-400 dark:border-amber-500' : BORDER_COLORS[i % BORDER_COLORS.length]} hover:shadow-md transition-all duration-300 animate-slide-up`} style={{ animationDelay: `${i * 60}ms` }}>
